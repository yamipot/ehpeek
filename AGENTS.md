...
- Follow the original-page DOM and component ownership boundary documented in `packages/ehpeek/src/eh/dom/index.ts`.
- Keep original E-H selectors, markup assumptions, and original-node transformations inside `packages/ehpeek/src/eh/dom`; components only consume detached data and managed nodes.
...
- Reuse the standard sizing classes in `packages/ehpeek/uno.config.mjs` (`xs` through `xl`) for controls, spacing, corners, and text before using arbitrary values. Reuse semantic colors from `packages/ehpeek/src/theme.css` and existing `ehp-color-*` shortcuts; avoid hardcoded colors and one-off color shortcuts.
