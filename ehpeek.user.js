...
.ehpeek-preview-host[data-embedded="false"] { position: fixed; inset: 0; z-index: 2300; touch-action: none; }
...
`;
  ...
  var DEFAULT_TAP_MOVE_THRESHOLD_PX, DEFAULT_DRAG_START_THRESHOLD_PX, DEFAULT_DRAG_INTENT_RATIO, VELOCITY_SAMPLE_WINDOW_MS, MOUSE_POINTER_ID, PointerGesture, init_chunk_ZG6TYGPK = __esm({
    "../reader/dist/chunk-ZG6TYGPK.js"() {
      "use strict";
      init_chunk_PKBMQBKP();
      init_solid();
      DEFAULT_TAP_MOVE_THRESHOLD_PX = 8, DEFAULT_DRAG_START_THRESHOLD_PX = 8, DEFAULT_DRAG_INTENT_RATIO = 1, VELOCITY_SAMPLE_WINDOW_MS = 60, MOUSE_POINTER_ID = -1, PointerGesture = class {
        constructor(target, callbacks) {
          __publicField(this, "pinchPointers", /* @__PURE__ */ new Map()), __publicField(this, "drag", null), __publicField(this, "holdTimer", null), __publicField(this, "suppressClick", !1), __publicField(this, "suppressClickTimer", null), __publicField(this, "pinch", null), __publicField(this, "onDragStart", (event) => {
            ...
            (canDrag || (callbacks2.shouldObserveTap?.(event) ?? !1)) && (this.start(event.pointerId, event.pointerType, event.clientX, event.clientY, event, canDrag), event.pointerType === "mouse" && this.addMouseListeners());
  ...
  // ../reader/dist/chunk-37NQQWFX.js
  var SCROLL_ANIMATION_MS, SCROLL_EASING_POWER, ANIMATION_FRAME_MIN_DELTA_MS, ANIMATION_FRAME_MAX_DELTA_MS, SCROLL_FLING_MIN_VELOCITY, SCROLL_FLING_STOP_VELOCITY, SCROLL_FLING_DECAY, VERTICAL_SCROLL_FLING_INITIAL_VELOCITY_FACTOR, ScrollAnimator, ScrollFlingAnimator, init_chunk_37NQQWFX = __esm({
    "../reader/dist/chunk-37NQQWFX.js"() {
      "use strict";
      init_chunk_E6UKP7HT();
  ...
  function createReaderGestures(options) {
    let ctx = useReaderContext(), { viewport, zoom } = options, [zoomImage, setZoomImage] = options.zoomImage, controls = () => getReaderControls(ctx), pagedMode = () => controls().navigationMode === "paged", lastZoomTap = null, pinchStart = null, startPinch = () => {
    ...
    }, isPageReloadButtonTarget = (event) => event.target instanceof Element && event.target.closest(".ehpeek-reader-page-reload") !== null, isPreviewSwipe = (info) => pagedMode() ? controls().direction === "ttb" ? Math.abs(info.dx) >= PAGED_PREVIEW_SWIPE_THRESHOLD && Math.abs(info.dy) <= PAGED_PREVIEW_SWIPE_AXIS_LIMIT : info.dy >= PAGED_PREVIEW_SWIPE_THRESHOLD && Math.abs(info.dx) <= PAGED_PREVIEW_SWIPE_AXIS_LIMIT : !1, runSingleTap = (info, event) => {
      if (zoomImage() !== null)
        event.preventDefault();
      ...
      shouldCaptureDrag: (event) => !(isPageReloadButtonTarget(event) || !(event instanceof PointerEvent) || event.pointerType === "mouse" && event.button !== 0),
  ...
  var PAGED_SWIPE_THRESHOLD, PAGED_PREVIEW_SWIPE_THRESHOLD, PAGED_PREVIEW_SWIPE_AXIS_LIMIT, MOUSE_HOLD_ZOOM_MS, ZOOM_DOUBLE_TAP_MS, ZOOM_DOUBLE_TAP_DISTANCE, ZOOM_DOUBLE_TAP_SCALE, TAP_CANCEL_DISTANCE, GHOST_CLICK_WINDOW_MS, init_chunk_UPMSF6PX = __esm({
    "../reader/dist/chunk-UPMSF6PX.js"() {
      "use strict";
      init_chunk_TRAUK5J2();
      init_chunk_E6UKP7HT();
      init_solid();
      PAGED_SWIPE_THRESHOLD = 24, PAGED_PREVIEW_SWIPE_THRESHOLD = 48, PAGED_PREVIEW_SWIPE_AXIS_LIMIT = 32, MOUSE_HOLD_ZOOM_MS = 350, ZOOM_DOUBLE_TAP_MS = 300, ZOOM_DOUBLE_TAP_DISTANCE = 36, ZOOM_DOUBLE_TAP_SCALE = 1.2, TAP_CANCEL_DISTANCE = 8, GHOST_CLICK_WINDOW_MS = 500;
    }
  });

  // ../reader/dist/chunk-55Q7FKT4.js
  function ReaderViewport(props) {
    let ctx = useReaderContext(), controls = () => getReaderControls(ctx), pageLayout = () => controls().pageLayout === "double" && controls().firstPageSeparate && ctx.position.page() === 1 ? "single" : controls().pageLayout, pagedMode = () => controls().navigationMode === "paged", initPageNum = untrack(() => props.initPage), horizontalAxis = () => controls().direction !== "ttb", [slots, setSlots] = createSignal([]), [revision, setRevision] = createSignal(0), [size, setSize] = createSignal(null), [firstVisiblePage, setFirstVisiblePage] = createSignal(null), [scrollOffset, setScrollOffset] = createSignal(0), [zoomImage, setZoomImage] = createSignal(null), scroller, scrollerApi, gestures, disposed = !1, resizeFrame, scrollFrame, programmaticFrame, programmatic = !1, syncingRevision = 0, pages = {
      items: []
    ...
    }, horizontalAnchorOffset = (pageSlots2, anchor2) => {
      let orderedSlots = controls().direction === "rtl" ? pageSlots2.slice().reverse() : pageSlots2, offset = 0;
      ...
      let scrolling = controls().navigationMode === "scroll", reference = scrolling ? ctx.scrollScale.referenceImageSize() : null, frame = pageFrameSize({
      ...
      !pagedMode() && !ctx.scrollScale.adjusting() && !zoomImage() && (horizontalAxis() ? moveToLeft(scrollerApi.scrollLeft()) : moveToTop(scrollTop())), setScrollOffset(horizontalAxis() ? scrollerApi.scrollLeft() : scrollTop()), !(programmatic || gestureDragging() || scrollFrame !== void 0) && (scrollFrame = window.requestAnimationFrame(() => untrack(() => {
        scrollFrame = void 0, followScroll();
      })));
    }, previousMode = "", previousDirection = "", previousPageLayout = "", previousViewportSize = null;
    createEffect(() => {
      let numbers = ctx.loading.windowPages(), mode = controls().navigationMode, direction = controls().direction, layout = pageLayout(), viewportSize = size(), anchorMoveRevision = moveRevision, anchor2 = settleMove ? null : scrollerApi.centerAnchor(), oldOffset = anchor2 && horizontalAxis() ? horizontalAnchorOffset(pages.items, anchor2) : null, oldLeft = scrollerApi.scrollLeft(), previous = new Map(pages.items.map((slot) => [slot.pageNum, slot])), previousSizes = pages.items.map((slot) => ({
        page: slot.pageNum,
        width: slot.frameWidth,
        height: slot.frameHeight
      }));
      pages.items = numbers.map((pageNum, index) => {
        let slot = previous.get(pageNum) ?? {
          pageNum,
          index,
        ...
        return slot.index = index, slot;
      });
      for (let slot of pages.items) applySlotSize(slot);
      if (!(previousMode !== mode || previousDirection !== direction || previousPageLayout !== layout || previousViewportSize?.width !== viewportSize?.width || previousViewportSize?.height !== viewportSize?.height || previousSizes.length !== pages.items.length || pages.items.some((slot, index) => {
        let old = previousSizes[index];
        return !old || old.page !== slot.pageNum || old.width !== slot.frameWidth || old.height !== slot.frameHeight;
      }))) return;
      previousMode = mode, previousDirection = direction, previousPageLayout = layout, previousViewportSize = viewportSize, setSlots(pages.items.slice()), refresh();
      let nextOffset = anchor2 && horizontalAxis() ? horizontalAnchorOffset(pages.items, anchor2) : null, token = ++syncingRevision;
      oldOffset !== null && nextOffset !== null && (suppressScrollObservation(), moveToLeft(oldLeft + nextOffset - oldOffset)), queueMicrotask(() => {
        disposed || token !== syncingRevision || (initialSeekActive && !settleMove ? moveToPage(initPageNum) : anchor2 && !settleMove && anchorMoveRevision === moveRevision && (suppressScrollObservation(), scrollerApi.restoreCenterAnchor(anchor2)), setFirstVisiblePage(measureFirstVisiblePage()));
      });
    }), createEffect(() => {
    ...
    return onMount(() => {
      ...
      return _el$2.addEventListener("wheel", (event) => gestures.wheel(event)), _el$2.addEventListener("scroll", onScroll), use((element) => {
        ...
        var _v$ = controls().navigationMode, _v$2 = controls().direction, _v$3 = pageLayout(), _v$4 = zoomImage() !== null, _v$5 = stripStyle();
        return _v$ !== _p$.e && setAttribute(_el$2, "data-navigation-mode", _p$.e = _v$), _v$2 !== _p$.t && setAttribute(_el$2, "data-read-direction", _p$.t = _v$2), _v$3 !== _p$.a && setAttribute(_el$2, "data-page-layout", _p$.a = _v$3), _v$4 !== _p$.o && setAttribute(_el$2, "data-zoom-active", _p$.o = _v$4), _p$.i = style(_el$3, _v$5, _p$.i), _p$;
      }, {
        e: void 0,
        t: void 0,
        a: void 0,
  ...
  function PageFrame(props) {
    ...
    return onCleanup(() => {
      ...
      var _el$4 = _tmpl$29(), _el$5 = _el$4.firstChild, _ref$ = node;
      return typeof _ref$ == "function" ? use(_ref$, _el$4) : node = _el$4, use((frame) => {
        props.slot.elements = {
          node,
          frame
        };
      }, _el$5), insert(_el$5, createComponent(ReaderPageView, {
        get pageNum() {
          return props.slot.pageNum;
        }
      })), createRenderEffect((_p$) => {
        var _v$6 = props.side, _v$7 = String(props.slot.pageNum), _v$8 = style2();
        return _v$6 !== _p$.e && setAttribute(_el$4, "data-pair-side", _p$.e = _v$6), _v$7 !== _p$.t && setAttribute(_el$4, "data-ehpeek-page-num", _p$.t = _v$7), _p$.a = style(_el$4, _v$8, _p$.a), _p$;
      }, {
        e: void 0,
        t: void 0,
        a: void 0
  ...
  var _tmpl$17, _tmpl$29, PAGE_SLOT_SPACING, FALLBACK_ASPECT_RATIO, HORIZONTAL_FLING_VELOCITY_MULTIPLIER, HORIZONTAL_FLING_MAX_VELOCITY, VERTICAL_FLING_VELOCITY_MULTIPLIER, VERTICAL_FLING_MAX_VELOCITY, VERTICAL_FLING_DECAY, VERTICAL_FLING_MAX_FRAME_DELTA_MS, init_chunk_55Q7FKT4 = __esm({
    "../reader/dist/chunk-55Q7FKT4.js"() {
      "use strict";
      init_chunk_6MQF4MFC();
      init_chunk_37NQQWFX();
      ...
      init_web();
      init_solid();
      _tmpl$17 = /* @__PURE__ */ template("<div class=ehpeek-reader-canvas><div class=ehpeek-reader-scroller tabindex=-1><main class=ehpeek-reader-page-strip>"), _tmpl$29 = /* @__PURE__ */ template("<section class=ehpeek-page><div class=ehpeek-reader-page-frame>"), PAGE_SLOT_SPACING = 8, FALLBACK_ASPECT_RATIO = 1.42, HORIZONTAL_FLING_VELOCITY_MULTIPLIER = 1.4, HORIZONTAL_FLING_MAX_VELOCITY = 1.8, VERTICAL_FLING_VELOCITY_MULTIPLIER = 1.35, VERTICAL_FLING_MAX_VELOCITY = 2.4, VERTICAL_FLING_DECAY = 1e-3, VERTICAL_FLING_MAX_FRAME_DELTA_MS = 96;
    }
  });

  // ../reader/dist/chunk-Z3FCOX44.js
  function Reader(props) {
    ...
    }), [page2, setPage] = createSignal(initial), [seeking, setSeeking] = createSignal(!1), [closing, setClosing] = createSignal(!1), [toolbarOpen, setToolbarOpen] = createSignal(!1), [viewport, setViewport] = createSignal(null), refs = {
      viewport,
    ...
    function gotoPage(pageNum, publish = !0) {
      ...
      publish || (lastReported = target), cancelTurn(), batch(() => {
        cancelSeek(), setNotifyIntent(publish), setPage(target);
    ...
    return createEffect(() => {
      ...
      disposed = !0, cancelTurn(), cancelSeek(), props.ref?.(null);
  ...
  var _tmpl$18, _tmpl$210, init_chunk_Z3FCOX44 = __esm({
    "../reader/dist/chunk-Z3FCOX44.js"() {
      "use strict";
      init_chunk_I4HV7TJW();
      init_chunk_PBZTMYEM();
      ...
      init_web();
      init_solid();
      _tmpl$18 = /* @__PURE__ */ template("<header class=ehpeek-reader-header>"), _tmpl$210 = /* @__PURE__ */ template("<div id=ehpeek-reader class=ehpeek-reader>");
    }
  });
  ...
  function createPreviewGestures(options) {
    let { preview } = options, horizontal = () => preview.settings[0].direction !== "ttb", enabled = () => preview.visible() && !preview.disabled(), [active, setActive] = createSignal(!1), [resizeAnchor, setResizeAnchor] = createSignal(null), fling = new ScrollFlingAnimator(), dragDirection = null, dragStartPosition = 0, cancelMotion = () => fling.cancel(), dismissOffset = 0, dismissAnimation = null, resetDismiss = () => {
      ...
      get dragAxis() {
        return preview.close ? "any" : horizontal() ? "x" : "y";
      },
      shouldCaptureDrag(event) {
        return !!preview.close || !("pointerType" in event) || event.pointerType === "mouse";
      },
      shouldObserveTap: () => !0,
      onStart() {
    ...
    }), { active, resizeAnchor, cancelMotion };
  }
  var HORIZONTAL_FLING_VELOCITY_FACTOR, init_chunk_LYIE6LUG = __esm({
    "../reader/dist/chunk-LYIE6LUG.js"() {
      "use strict";
      init_chunk_O4OUM2TM();
      init_chunk_37NQQWFX();
      ...
      init_chunk_E6UKP7HT();
      init_solid();
      HORIZONTAL_FLING_VELOCITY_FACTOR = 1.6;
    }
  });

  // ../reader/dist/chunk-2N7YTWHY.js
  ...
  function Viewport(props) {
    ...
    }), horizontal ? MAX_TILE_WIDTH * estimatedAspectRatio : MAX_TILE_WIDTH), scroller, [layout, setLayout] = createSignal(null), [scrollOffset, setScrollOffset] = createSignal(0), [ready, setReady] = createSignal(!1), [positionBarActive, setPositionBarActive] = createSignal(!1), gestures = createPreviewGestures({
    ...
    }, scrollFrame = null, reportScroll = () => {
      layoutFrame !== null || scrollFrame !== null || (scrollFrame = window.requestAnimationFrame(() => {
    ...
    onMount(() => {
      ...
      })), createEffect(on([ctx.previewCache.version, () => gestures.active() || positionBarActive(), ctx.visible], ([version, interacting, visible], previous) => {
      ...
      observer.observe(scroller), onCleanup(() => {
        observer.disconnect(), layoutFrame !== null && window.cancelAnimationFrame(layoutFrame), scrollFrame !== null && window.cancelAnimationFrame(scrollFrame), ctx.refs.bindViewport(null);
      });
    });
    ...
    return (() => {
      ...
      return typeof _ref$ == "function" ? use(_ref$, _el$2) : scroller = _el$2, _el$2.classList.toggle("ehpeek-preview-scroller--horizontal", !!horizontal), insert(_el$2, createComponent(Show, {
        ...
        var _v$ = `ehpeek-preview-viewport${props.class ? ` ${props.class}` : ""}`, _v$2 = props.style, _v$3 = ctx.close ? "none" : "pan-x pan-y", _v$4 = ctx.close ? "contain" : "auto";
  ...
  var _tmpl$21, GRID_GAP, MAX_TILE_WIDTH, OVERSCAN_GROUPS, init_chunk_2N7YTWHY = __esm({
    "../reader/dist/chunk-2N7YTWHY.js"() {
      "use strict";
      init_chunk_WGECFBEU();
      init_chunk_YD563ASB();
      ...
      init_web();
      init_solid();
      _tmpl$21 = /* @__PURE__ */ template("<div><div class=ehpeek-preview-scroller>"), GRID_GAP = 8, MAX_TILE_WIDTH = 220, OVERSCAN_GROUPS = 4;
    }
  });

  // ../reader/dist/chunk-MXRD3IEY.js
  ...
  var _tmpl$30, ScrollPreview, init_chunk_MXRD3IEY = __esm({
    "../reader/dist/chunk-MXRD3IEY.js"() {
      "use strict";
      init_chunk_R253JGHC();
      init_chunk_IECOMI6N();
