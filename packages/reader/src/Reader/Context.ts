import { createContext, useContext, type Accessor, type Setter } from "solid-js";
import type {
  ContentSource,
  ReaderOrientation,
  ReaderCustomization,
  ReaderSettingsState,
} from "../kit/interfaces";
import type { ReaderPosition } from "./index";
import type { ReaderLoading } from "./loading";
import type { ReaderScrollScale } from "./ScrollScale";
import type { ReaderViewportRef } from "./Viewport";

/** One Reader subtree; viewport geometry and gesture snapshots stay with their owners. */
export interface ReaderContext {
  source: ContentSource;
  settings: ReaderSettingsState;
  /** Selects portrait or landscape preferences as the screen rotates. */
  orientation: Accessor<ReaderOrientation>;
  customization: ReaderCustomization;
  position: ReaderPosition;
  loading: ReaderLoading;
  scrollScale: ReaderScrollScale;
  /** Pairing choice for this mount only; it is not a persisted preference. */
  firstPageSeparate: [Accessor<boolean>, Setter<boolean>];
  disabled: Accessor<boolean>;
  refs: {
    /** Viewport-owned geometry and motion; null while the component is absent. */
    viewport: Accessor<ReaderViewportRef | null>;
    /** Navigation targets the mounted viewport; null disconnects it on unmount. */
    bindViewport(viewport: ReaderViewportRef | null): void;
  };
  /** Dismiss image zoom first, otherwise ask the host to close Reader. */
  close(): void;
  /** Open Preview at the displayed reading page. */
  openPreview(): void;
  /** Report end-screen activation and request closure. */
  finish(): void;
}

export const ReaderContextKey = createContext<ReaderContext>();
export function useReaderContext(): ReaderContext {
  const ctx = useContext(ReaderContextKey);
  if (!ctx) throw new Error("Reader components require Reader context");
  return ctx;
}

/** Effective mode and direction; the two saved directions remain independent. */
export function getReaderControls(ctx: ReaderContext) {
  const settings = ctx.settings[`${ctx.orientation()}Controls`];
  const navigationMode = settings.navigationMode.value();
  return {
    navigationMode,
    direction: navigationMode === "scroll" ? settings.scrollDirection.value() : settings.pagedDirection.value(),
    pageLayout: settings.pageLayout.value(),
    rightTapAction: settings.rightTapAction.value(),
    firstPageSeparate: ctx.firstPageSeparate[0](),
  };
}
