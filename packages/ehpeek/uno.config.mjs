...
import { createGenerator, defineConfig, expandVariantGroup, presetWind3, transformerVariantGroup } from "unocss";

const SIZE_NAMES = ["xs", "sm", "md", "lg", "xl"];

function createUiUnoConfig(pointerSelector) {
  return defineConfig({
    presets: [presetWind3()],
    transformers: [transformerVariantGroup()],
    postprocess: [pointerHoverPostprocessor(pointerSelector)],
    variants: [
    ...
    shortcuts: {
      "ehp-color-reader":
        "bg-[var(--color-reader-background)] text-[var(--color-reader-text)]",
      "ehp-color-spinner":
      ...
      "ehp-color-site-border-subtle-b":
        "border-b-[var(--color-site-border-subtle)]",
      "ehp-color-site-elevated":
        "bg-[var(--color-site-elevated)] shadow-[0_8px_24px_var(--color-shadow-elevated)]",
      ...
      "z-ui": "z-1000",
      "z-overlay": "z-1100",
      "z-reader-panel": "z-900",
      "z-reader": "z-1200",
      "z-dialog": "z-1400",
      ...pixelShortcuts(["w", "h", "min-h"], {
        xs: 24,
        sm: 32,
        md: 40,
        lg: 52,
...
function hitSizeShortcuts() {
  return Object.fromEntries(
    SIZE_NAMES.flatMap((name) => [
      ...
      [
        `ui-hit-square-${name}`,
        `w-[var(--ui-hit-size-${name})] h-[var(--ui-hit-size-${name})]`,
      ],
    ]),
  );
}
...
function pointerHoverPostprocessor(pointerSelector) {
  return (utility) => {
    if (utility.selector.includes(":hover")) {
      utility.selector = `${pointerSelector} ${utility.selector}`;
    }
  };
}
...
export async function generateCss(sourceDirs, config) {
  ...
  const content = sourceDirs
    ...
    .join("\n");
  return (
    await generator.generate(expandVariantGroup(content), { preflights: true })
  ).css;
}
