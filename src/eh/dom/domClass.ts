...
const common = group({
    ...
    image: image("img"),
    interactive: query("a[href], button, input, select, textarea, label, [onclick]"),
    links: anchor("a[href]"),
    scripts: script("script"),
});

const ehSyringe = group({
...
const gallery = group({
    ...
    preview: group({
      description: cls("gpc"),
      imageLinks: anchor("#gdt a[href], .gdtm a[href], .gdtl a[href], a[href*='/s/']"),
...
const search = group({
    ...
    navigationLink: anchor(".searchnav a[id$='first'][href], .searchnav a[id$='prev'][href], .searchnav a[id$='next'][href], .searchnav a[id$='last'][href]"),
    panel: group({
      box: id("searchbox", {
        apply: {
