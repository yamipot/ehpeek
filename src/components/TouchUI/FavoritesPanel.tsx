      ...
      <Show when={open()}>
        <div class="border-0 border-t border-t-[var(--color-site-border-subtle)]">
          <For each={props.source.data.favoritesCategory?.categories}>{(category, index) => (
            <button
              type="button"
              class={`flex box-border w-full min-h-xs large:min-h-md items-center px-sm large:px-md py-xs large:py-sm border-0 border-b ehp-color-site-border-subtle-b last:border-b-0 text-left textsize-sm large:textsize-md font-inherit no-underline cursor-pointer ${category.selected ? "bg-[var(--color-site-accent-hover)] ehp-color-site-accent font-700" : "!bg-transparent ehp-color-site-text hover:!bg-[var(--color-site-item-hover)]"}`}
