...
export function FavoritesCategorySelect(props: { source: TouchResultsPageDom }) {
  ...
  return (
    <div ref={container} class="box-border w-full min-w-0 overflow-hidden ui-rounded-xs border ehp-color-site-border bg-[var(--color-site-elevated)]">
      <button
        type="button"
        class="flex box-border w-full min-h-[max(32px,var(--ui-control-size-xs))] items-center justify-between ui-gap-sm ui-px-sm ui-py-xs ui-rounded-xs border-0 !bg-transparent ehp-color-site-text text-left textsize-sm font-700 font-inherit cursor-pointer hover:!bg-[var(--color-site-item-hover)] active:!bg-[var(--color-site-item-hover)]"
        aria-expanded={open()}
      ...
      <Show when={open()}>
        <div class="border-0 border-t border-t-[var(--color-site-border-subtle)]">
          <For each={props.source.data.favoritesCategory?.categories}>{(category, index) => (
            <button
              type="button"
              class={`flex box-border w-full min-h-[max(32px,var(--ui-control-size-xs))] items-center ui-px-sm ui-py-xs border-0 border-b ehp-color-site-border-subtle-b last:border-b-0 text-left textsize-sm font-inherit no-underline cursor-pointer ${category.selected ? "bg-[var(--color-site-accent-hover)] ehp-color-site-accent font-700" : "!bg-transparent ehp-color-site-text hover:!bg-[var(--color-site-item-hover)]"}`}
